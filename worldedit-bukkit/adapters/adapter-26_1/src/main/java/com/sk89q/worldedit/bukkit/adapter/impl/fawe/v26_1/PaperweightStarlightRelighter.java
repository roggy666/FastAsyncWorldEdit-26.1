...
public class PaperweightStarlightRelighter extends StarlightRelighter<ServerLevel, ChunkPos> {
    ...
    private static final TicketType FAWE_TICKET = new TicketType(
    ...
    protected CompletableFuture<?> chunkLoadFuture(final ChunkPos chunkPos) {
        ...
        serverLevel.getWorld().getChunkAtAsync(chunkPos.x(), chunkPos.z(), true, true, future::complete);
        return future.thenAccept(c -> serverLevel.getChunkSource().addTicketAtLevel(
                        FAWE_TICKET,
                        chunkPos,
                        LIGHT_LEVEL
                ));
    ...
    protected void invokeRelight(
        ...
        try {
            // Starlight serverRelightChunks is removed in 26.1; use per-chunk lightChunk as fallback
            var lightEngine = serverLevel.getChunkSource().getLightEngine();
            int count = 0;
            for (ChunkPos pos : coords) {
                var chunk = serverLevel.getChunkSource().getChunk(pos.x(), pos.z(), false);
                if (chunk != null) {
                    lightEngine.lightChunk(chunk, false);
                }
                chunkCallback.accept(pos);
                count++;
            }
            processCallback.accept(count);
        } catch (Exception e) {
